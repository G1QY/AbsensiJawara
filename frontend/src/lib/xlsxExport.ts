...
export function makeWorkbook(rows:(string|number|null)[][]):Uint8Array {
  ...
  const files:Record<string,string>={
    ...
    'xl/worksheets/sheet1.xml':`<worksheet xmlns="${ns}"><dimension ref="A1:${end}"/><sheetViews><sheetView workbookViewId="0"><pane ySplit="1" topLeftCell="A2" activePane="bottomLeft" state="frozen"/></sheetView></sheetViews><cols><col min="1" max="${Math.max(1,...rows.map(row=>row.length))}" width="24" customWidth="1"/></cols><sheetData>${sheetRows}</sheetData><autoFilter ref="A1:${end}"/></worksheet>`
  };
