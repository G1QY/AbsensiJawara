...
export function MiniChart({title,subtitle,points,color='#1554df',format=String}:{title:string;subtitle?:string;points:{label:string;value:number}[];color?:string;format?:(n:number)=>string}) {
  ...
  return <section className={panel}><h3 className="text-sm font-semibold text-slate-900">{translateUI(title)}</h3>{subtitle&&<p className="text-xs text-slate-500 mt-1">{translateUI(subtitle)}</p>}<div className="flex gap-3 items-end h-44 mt-5" role="img" aria-label={translateUI(title)+': '+points.map(p=>`${translateUI(p.label)} ${format(p.value)}`).join(', ')}>{points.map(p=><div key={p.label} className="flex flex-col justify-end items-center h-full flex-1 min-w-0"><span className="text-[10px] text-slate-600 mb-2 truncate max-w-full">{format(p.value)}</span><div className="w-full max-w-9 rounded-t-md" style={{height:`${p.value/max*105}px`,minHeight:p.value?3:1,background:color}}/><span className="text-[10px] text-slate-500 mt-2">{translateUI(p.label)}</span></div>)}</div></section>;
}
