...
export default function AdminAbsensi() {
  ...
  function photos(a:AttendanceRow) {
    return <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">{(['in','out'] as const).map(side=>{
      ...
      return <section key={side} className="p-4 rounded-xl bg-slate-50 space-y-3">
        ...
        {photo?<img src={photo} alt={side==='in'?translateUI("Foto Clock In"):translateUI("Foto Clock Out")} className="w-full rounded-xl object-contain max-h-80" onError={e=>{e.currentTarget.hidden=true;e.currentTarget.insertAdjacentText('afterend','Foto gagal dimuat. Tutup dan buka ulang detail.');}}/>:<p className="text-sm text-slate-500 border border-dashed border-slate-300 rounded-xl p-5">{time?translateUI("Foto tidak tersedia untuk catatan ini."):`Belum ada foto ${side==='in'?'Clock In':'Clock Out'}.`}</p>}
        <p className="text-sm font-semibold text-slate-900">{stamp(time)}</p>
        <div className="text-sm text-slate-700 whitespace-pre-wrap break-words"><p className="font-semibold">{translateUI("Catatan") + " "}{side==='in'?'Clock In':'Clock Out'}</p><p>{comment||translateUI("Tidak ada catatan.")}</p></div>
      </section>;
    })}</div>;
  }
  ...
  function calculation(a:AttendanceRow){
    ...
    return <div className="grid sm:grid-cols-3 gap-3"><div className="rounded-xl bg-blue-50 p-4"><p className="text-xs text-blue-700">{translateUI("Status Clock In")}</p><p className="font-bold text-slate-900 mt-1">{a.status}</p><p className="text-xs text-slate-600 mt-1">{a.lateMinutes===null?translateUI("Tidak dapat dihitung tanpa jadwal"):a.lateMinutes?`${a.lateMinutes} menit telat`:translateUI("Tepat waktu / lebih awal")}</p></div><div className="rounded-xl bg-red-50 p-4"><p className="text-xs text-red-700">{translateUI("Potongan Telat")}</p><p className="font-bold text-red-700 mt-1">{deduction===null?translateUI("Belum dapat dihitung"):'-'+money(deduction)}</p><p className="text-xs text-slate-600 mt-1">{lateHours===null?translateUI("Guest/tanpa jadwal"):`${lateHours} jam × ${money(rules.lateRate)}`}</p></div><div className="rounded-xl bg-emerald-50 p-4"><p className="text-xs text-emerald-700">{translateUI("Bonus Lembur")}</p><p className="font-bold text-emerald-700 mt-1">{bonus===null?translateUI("Belum dapat dihitung"):'+'+money(bonus)}</p><p className="text-xs text-slate-600 mt-1">{overtimeHours===null?translateUI("Belum Clock Out / tanpa jadwal"):approved?`${overtimeHours} jam × ${money(rules.overtimeRate)}`:`${overtimeHours} jam • ${reviewLabel(a.overtimeStatus)}`}</p></div></div>;
  }
  return <div className="p-4 sm:p-6 space-y-5">
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-max text-sm"><thead className="text-xs bg-slate-50 text-slate-600"><tr>{['Nama & Kontak','Perusahaan','Jabatan','Jenis Penugasan','Lokasi','Tanggal','Jadwal','Clock In','Status','Telat','Clock Out','Lembur','Persetujuan Absensi','Persetujuan Lembur','Aksi'].map(h=><th key={h} className={`text-left p-3 whitespace-nowrap ${h==='Aksi'?'sticky right-0 bg-slate-50 z-10':''}`}>{translateUI(h)}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{filtered.map(a=><tr key={a.source+':'+a.id}>
      ...
      <td className="p-3 whitespace-nowrap text-slate-700">{a.kind}</td><td className="p-3 min-w-40 text-slate-700">{a.location}</td><td className="p-3 text-slate-700">{a.date||translateUI("Tanggal belum terbaca")}</td><td className="p-3 min-w-36 text-slate-600">{scheduleLabel(a.schedule)}</td>
      <td className="p-3 min-w-36 text-slate-700">{stamp(a.clockIn)}</td><td className="p-3 text-slate-700">{translateUI(a.status)}</td><td className="p-3 text-slate-700">{a.lateMinutes===null?translateUI("Belum dapat dihitung"):`${a.lateMinutes} menit`}</td>
      <td className="p-3 min-w-36 text-slate-700">{stamp(a.clockOut)}</td><td className="p-3 min-w-28 text-slate-700">{a.overtimeMinutes===null?translateUI("Belum dihitung"):`${a.overtimeMinutes} menit`}</td>
    ...
    <Modal open={!!detail} onClose={()=>setDetail(null)} title={translateUI("Detail Absensi")} size="lg">{detail&&<div className="space-y-4">{identity(detail)}<p className="text-sm text-slate-600">{translateUI("Jadwal:") + " "}{scheduleLabel(detail.schedule)} • {detail.date||translateUI("Tanggal belum terbaca")}</p>{calculation(detail)}{positions(detail)}{photos(detail)}<p className="text-sm text-slate-700 whitespace-pre-wrap">{translateUI("Catatan admin:") + " "}{detail.reviewNote||translateUI("Tidak ada catatan.")}</p><p className="text-xs text-slate-500">{detail.timeSource==='LEGACY_DEVICE'?translateUI("Waktu berasal dari perangkat pada data lama. Bukan waktu server."):translateUI("Waktu penerimaan absensi berasal dari server.")}{" " + translateUI("Foto masuk dan pulang tidak dipasangkan otomatis berdasarkan nama/HP guest.")}</p></div>}</Modal>
