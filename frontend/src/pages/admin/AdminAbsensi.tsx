...
export default function AdminAbsensi() {
  ...
  function positions(a:AttendanceRow) {
    return <div className="grid sm:grid-cols-2 gap-3">{(['in','out'] as const).map(side=>{
      ...
      return <div key={side} className="rounded-xl border border-slate-200 p-3 text-xs text-slate-700">
        ...
        {point?<><p className="mt-1">{point.latitude}, {point.longitude}</p><p>Akurasi: {point.accuracy===null?'Tidak tercatat':`±${point.accuracy} m`}</p><a className="text-blue-700 underline" href={gpsLink(point)} target="_blank" rel="noopener noreferrer">Buka titik GPS</a></>:<p>Koordinat belum tercatat.</p>}
      </div>;
    })}</div>;
  }
  ...
  return <div className="p-4 sm:p-6 space-y-5">
    <div className="flex flex-wrap justify-between gap-3 items-center"><div><h2 className="font-bold text-lg text-slate-900">Rekap Absensi</h2><p className="text-sm text-slate-600">Kehadiran, catatan crew, dan persetujuan lembur.</p></div><div className="flex flex-wrap gap-2"><button className={button} disabled={loading||busy} onClick={()=>void load()}>Muat ulang</button><ExportButtons filename="Rekap-Absensi" title="Rekap Absensi" subtitle="Sesuai filter aktif, waktu WIB" headers={recap(filtered)[0] as string[]} rows={recap(filtered).slice(1)} /></div></div>
    ...
    <p className="text-xs text-slate-600">Ringkasan, Excel, dan PDF mengikuti seluruh filter di bawah.</p>
    ...
    <div className="bg-white border border-slate-200 rounded-xl overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-max text-sm"><thead className="text-xs bg-slate-50 text-slate-600"><tr>{['Nama & Kontak','Perusahaan','Jabatan','Jenis Penugasan','Lokasi','Tanggal','Jadwal','Clock In','Status','Telat','Clock Out','Lembur','Persetujuan Absensi','Persetujuan Lembur','Aksi'].map(h=><th key={h} className={`text-left p-3 whitespace-nowrap ${h==='Aksi'?'sticky right-0 bg-slate-50 z-10':''}`}>{h}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{filtered.map(a=><tr key={a.source+':'+a.id}>
      ...
      <td className="p-3 text-slate-700">{a.companyName || 'Belum diisi'}</td><td className="p-3 text-slate-700">{a.jobTitle || 'Belum diisi'}</td>
