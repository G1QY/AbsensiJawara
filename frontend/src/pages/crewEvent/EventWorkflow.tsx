...
const fmt = (n: number) => 'Rp' + n.toLocaleString(getLocale());
...
const eventDateText = (value?: string) => value ? new Date(`${value}T00:00:00+07:00`).toLocaleDateString(getLocale(), { day: 'numeric', month: 'long', year: 'numeric', timeZone: 'Asia/Jakarta' }) : '-';
...
async function exportEventPDF(assignment: CrewEventAssignment, data: Record<string, any> = {}, download = true, photoBase = '/crew-event/events') {
  ...
  const info = [
    ...
    ['Tanggal Export (WIB)', new Date().toLocaleDateString(getLocale(), { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric', timeZone: 'Asia/Jakarta' })],
  ...
  for (let i = 1; i <= totalPages; i++) {
    ...
    doc.text(new Date().toLocaleString(getLocale()), pageWidth - 14, doc.internal.pageSize.getHeight() - 10, { align: 'right' });
  }
  ...
  return (
    <div className="bg-white border border-slate-100 rounded-2xl p-5">
      <div className="flex items-center justify-between mb-3">
        <div>
          <h3 className="font-semibold text-slate-900 text-sm">{translateUI("Progress Workflow")}</h3>
      ...
      <label className="block sm:hidden text-sm text-slate-600">{translateUI("Langkah kerja")}<select aria-label={translateUI("Pilih langkah workflow")} className="mt-2 w-full rounded-xl border border-slate-200 p-3 bg-white" value={current} onChange={event => onGoToStep(Number(event.target.value))}>{steps.map(step => <option key={step.id} value={step.id} disabled={step.id > Math.max(current, maxReached)}>{step.id}. {translateUI(step.label)}</option>)}</select></label>
      ...
      <p className="hidden sm:block text-xs text-slate-500 mt-2">{translateUI("Pilih nomor langkah untuk navigasi")}</p>
    </div>
  );
}
        ...
        {items.map((item: any, i: number) => (
          <div key={i} className="bg-slate-50 rounded-xl p-4 space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">{translateUI("Nama Barang")}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">{translateUI("Jumlah")}</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">{translateUI("Kategori")}</label>
                <select value={item.kategori} onChange={e => { const n = [...items]; n[i] = { ...n[i], kategori: e.target.value }; setItems(n); }} className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                  ...
                  <option value={"Non Electronic"}>{translateUI("Non Electronic")}</option>
                </select>
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1">{translateUI("Kondisi")}</label>
                <select value={item.kondisi} onChange={e => { const n = [...items]; n[i] = { ...n[i], kondisi: e.target.value }; setItems(n); }} className="w-full px-3 py-2 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
                  <option value={"Baik"}>{translateUI("Baik")}</option>
                  <option value={"Rusak Ringan"}>{translateUI("Rusak Ringan")}</option>
                  <option value={"Rusak Berat"}>{translateUI("Rusak Berat")}</option>
                </select>
              </div>
            </div>
            <PhotoPicker fieldLabel="Foto Barang" label={translateUI("Buka kamera")} value={item.foto} onChange={key => { const n = [...items]; n[i] = { ...n[i], foto: key }; setItems(n); }} />
            {items.length > 1 && (
              <button onClick={() => { const n = items.filter((_: any, idx: number) => idx !== i); setItems(n); }} className="text-xs text-red-500 hover:text-red-700 font-medium">{translateUI("Hapus barang ini")}</button>
            )}
          </div>
        ))}
      </div>

      <button onClick={() => setItems([...items, { nama: '', jumlah: 1, kategori: 'Electronic', kondisi: 'Baik' }])} className="w-full py-2.5 rounded-xl border-2 border-dashed border-slate-200 text-sm text-slate-500 hover:border-blue-300 hover:text-blue-600 font-medium">{" " + translateUI("+ Tambah Barang") + " "}</button>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          <h3 className="font-semibold text-slate-900">{translateUI("Transportasi") + " "}{type === 'pergi' ? translateUI("Pergi") : translateUI("Pulang")}</h3>
          <p className="text-xs text-slate-400">{translateUI("Catat perjalanan, kendaraan, waktu, dan biaya")}</p>
        </div>
      </div>

      <div className="space-y-4">
        <div className="grid sm:grid-cols-2 gap-3">
          <label className="block text-sm font-medium text-slate-700">{translateUI("Tanggal")}<input type="date" value={data[dateKey] || defaultDate} onChange={e => onDataChange({ ...data, [dateKey]: e.target.value })} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></label>
          <label className="block text-sm font-medium text-slate-700">{translateUI("Jam") + " "}{type === 'pergi' ? translateUI("Keberangkatan") : translateUI("Kepulangan")}<input type="time" value={data[timeKey] || ''} onChange={e => onDataChange({ ...data, [timeKey]: e.target.value })} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></label>
          <label className="block text-sm font-medium text-slate-700">{translateUI("Layanan")}<select value={data[serviceKey] || 'Lalamove'} onChange={e => onDataChange({ ...data, [serviceKey]: e.target.value })} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm"><option value={"Lalamove"}>Lalamove</option><option value={"Transportasi Online"}>{translateUI("Transportasi Online")}</option><option value={"Kendaraan Pribadi"}>{translateUI("Kendaraan Pribadi")}</option><option value={"Sewa"}>{translateUI("Sewa")}</option><option value={"Lainnya"}>{translateUI("Lainnya")}</option></select></label>
          <label className="block text-sm font-medium text-slate-700">{translateUI("Kendaraan")}<select value={data[vehicleKey] || ''} onChange={e => onDataChange({ ...data, [vehicleKey]: e.target.value })} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-sm"><option value="">{translateUI("Pilih kendaraan")}</option><option value={"Motor"}>{translateUI("Motor")}</option><option value={"Mobil"}>{translateUI("Mobil")}</option><option value={"Van atau Pickup"}>{translateUI("Van atau Pickup")}</option><option value={"Truk"}>{translateUI("Truk")}</option><option value={"Lainnya"}>{translateUI("Lainnya")}</option></select></label>
        </div>
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1.5">{translateUI("Nominal Biaya")}</label>
        ...
        <PhotoPicker fieldLabel="Foto Bukti Pembayaran" label={translateUI("Buka kamera")} value={data[`transportasi_${type}_photo`]} onChange={photo => onDataChange({ ...data, [`transportasi_${type}_photo`]: photo })} />
      </div>
...
function StepCheckpoint({ type, onSave, onNext, data, onDataChange, locationName }: {
  ...
  return <div className="space-y-4">
    ...
    />
    {data[timeKey] ? <p className="rounded-xl bg-slate-50 px-3 py-2 text-xs text-slate-600">{translateUI("Tercatat pada") + " "}{new Date(data[timeKey]).toLocaleString(getLocale(), { timeZone: 'Asia/Jakarta' })} WIB</p> : null}
    <StepActions onSave={onSave} onNext={onNext} />
  </div>;
}
...
function StepTesPrint({ phase, onSave, onNext, data, onDataChange }: {
  ...
  return (
    <div className="space-y-4">
      ...
      <PhotoPicker fieldLabel={`Foto Cek Print ${isBefore ? 'Sebelum' : 'Sesudah'} Event`} label={translateUI("Buka kamera")} value={data[key] || (isBefore ? data.tes_print_photo : '')} onChange={photo => onDataChange({ ...data, [key]: photo })} />
      <StepActions onSave={onSave} onNext={onNext} />
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="text-center py-6">
        ...
        <p className="text-slate-500 text-sm">{isFinished ? translateUI("Status diambil dari data event dan absensi terbaru.") : translateUI("Fokus pada event. Clock Out saat event selesai.")}</p>
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-10 h-10 rounded-xl bg-blue-100 text-blue-700 flex items-center justify-center"><WorkflowIcon /></div>
        <div>
          <h3 className="font-semibold text-slate-900">{translateUI(title)}</h3>
          <p className="text-xs text-slate-400">{translateUI("Input nominal") + " "}{title.toLowerCase()}</p>
        </div>
      </div>
      {isQuota ? <div className="grid sm:grid-cols-2 gap-3">
        <label className="block text-sm font-medium text-slate-700">{translateUI("Jumlah Kuota (GB)")}<input type="number" min={0} step="0.1" value={data.kuota_gb ?? ''} onChange={e => onDataChange({ ...data, kuota_gb: draftNumber(e.target.value) })} placeholder={translateUI("Contoh: 50")} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></label>
        <label className="block text-sm font-medium text-slate-700">{translateUI("Provider / Perangkat")}<input type="text" value={data.kuota_provider || ''} onChange={e => onDataChange({ ...data, kuota_provider: e.target.value })} placeholder={translateUI("Contoh: Orbit Telkomsel")} className="mt-1.5 w-full px-3 py-2.5 rounded-xl border border-slate-200 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></label>
        <div className="sm:col-span-2">
          <label className="block text-sm font-medium text-slate-700 mb-1.5">{translateUI("Biaya Kuota")}</label>
          <div className="relative">
        ...
        <p className="sm:col-span-2 rounded-xl bg-slate-50 px-3 py-2 text-sm text-slate-600">{translateUI("Total omset:") + " "}<strong>{fmt(omsetTunai + omsetTransfer)}</strong></p>
      </div>}
    ...
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-xs text-slate-400">{translateUI("Input kondisi barang setelah event")}</p>
        </div>
      </div>
      <div className="space-y-3">
        {afterItems.map((item: any, i: number) => {
          ...
          return (
            <div key={i} className="bg-slate-50 rounded-xl p-4 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">{translateUI("Jumlah After")}</label>
                  <input
                    type="number"
                    min={0}
                    value={item.jumlah ?? ''}
                ...
                <div>
                  <label className="text-xs text-slate-500 mb-1 block">{translateUI("Kondisi")}</label>
                  <select
              ...
              {item.jumlah < beforeItem.jumlah && (
                <div>
                  <label className="text-xs text-red-600 mb-1 block font-medium">{translateUI("Alasan Selisih (wajib) *")}</label>
                  <input
                    type="text"
                    placeholder={translateUI("Jelaskan mengapa ada selisih...")}
                    value={item.alasan || ''}
              ...
              <PhotoPicker fieldLabel="Foto Barang Setelah Event" label={translateUI("Buka kamera")} value={item.foto_after} onChange={photo => { const n = [...afterItems]; n[i] = { ...n[i], foto_after: photo }; setAfterItems(n); }} />
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-3 mb-2">
        <div className="w-10 h-10 rounded-xl bg-blue-100 text-blue-700 flex items-center justify-center"><WorkflowIcon /></div>
        <div>
          <h3 className="font-semibold text-slate-900">{translateUI("Perbandingan Inventory")}</h3>
          <p className="text-xs text-slate-400">{translateUI("Before vs After — cek selisih barang")}</p>
        </div>
      </div>
          ...
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              {['Barang', 'Before', 'After', 'Selisih', 'Status'].map(h => (
                <th key={h} className="text-left px-4 py-3 text-xs font-semibold text-slate-400 uppercase">{translateUI(h)}</th>
              ))}
            </tr>
          </thead>
                ...
                <tr key={i} className={selisih < 0 ? 'bg-red-50/50' : ''}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs font-semibold px-2 py-1 rounded-lg ${selisih === 0 ? 'bg-emerald-50 text-emerald-700' : 'bg-red-50 text-red-700'}`}>
                      {selisih === 0 ? translateUI("Sesuai") : translateUI("Kurang")}
                    </span>
                  </td>
                </tr>
              );
      ...
      {comparison.some((c: any) => c.after < c.before) && (
        <div className="bg-red-50 border border-red-200 rounded-xl p-4">
          <p className="text-sm font-semibold text-red-700 mb-2">{translateUI("Ada selisih inventori!")}</p>
...
export default function EventWorkflow({ onBack, assignment, initialStep = 1 }: EventWorkflowProps) {
  ...
  const renderStep = () => {
    ...
    switch (currentStep) {
      ...
      case 9: return <StepKuotaOmset title={translateUI("Kuota/Orbit")} {...props} />;
      case 10: return <StepTransportasi type="pulang" defaultDate={lastEventDate} {...props} />;
      case 11: return <StepKuotaOmset title={translateUI("Omset Event")} {...props} />;
  ...
  if (maxReached >= TOTAL_STEPS || assignment.workflow.max_reached >= TOTAL_STEPS || ['COMPLETED','CANCELLED'].includes(assignment.event.status) || assignment.status !== 'ACTIVE') return <div className="p-4 sm:p-6 space-y-5">
    <section className="rounded-2xl border border-slate-200 bg-white p-5 space-y-3"><button onClick={onBack} className="text-sm text-blue-700">{translateUI("Kembali ke Event Saya")}</button><div className="flex flex-wrap justify-between items-center gap-3"><div><h1 className="text-xl font-semibold">{translateUI("Tinjau Workflow")}</h1><p className="text-sm text-slate-500">{eventName}</p></div><button disabled={exporting} onClick={handleExportPDF} className="rounded-xl border border-slate-200 px-4 py-2 text-sm">{exporting?translateUI("Menyiapkan…"):translateUI("Export PDF")}</button></div><p className="rounded-xl bg-emerald-50 p-3 text-sm text-emerald-800">{translateUI("Data workflow sudah dikunci. Anda dapat meninjau hasil dan bukti yang tersimpan.")}</p></section>
    {saveError && <p role="alert" className="text-red-700">{saveError}</p>}
    <section className="rounded-2xl border border-slate-200 bg-white p-4"><h2 className="mb-3 font-semibold">{translateUI("Bukti Kehadiran Event")}</h2><div className="grid sm:grid-cols-2 gap-4"><EventPhoto eventId={eventId} value={workflowData[`setup_ready_photo_${assignment.id}`]} label="Setup Ready"/><EventPhoto eventId={eventId} value={workflowData[`event_finished_photo_${assignment.id}`]} label={translateUI("Event Selesai")}/></div></section>
    <div className="flex gap-2">{(['Inventory','Operasional'] as const).map(tab=><button key={tab} onClick={()=>setReviewTab(tab)} className={`rounded-xl px-4 py-2 text-sm ${reviewTab===tab?'bg-blue-600 text-white':'border border-slate-200 bg-white'}`}>{translateUI(tab)}</button>)}</div>
  ...
  return (
    <WorkflowEventContext.Provider value={eventId}><WorkflowAssignmentContext.Provider value={assignment.id}><div className="event-workflow p-3 sm:p-6 space-y-4 sm:space-y-5 max-w-2xl mx-auto">
      ...
      <button onClick={handleSave} className="flex items-center gap-1.5 text-sm text-slate-500 hover:text-slate-700">
        <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>{" " + translateUI("Simpan & Kembali") + " "}</button>
      ...
      {currentStep < TOTAL_STEPS && (
        <div className="flex items-center gap-2 px-4 py-2.5 bg-blue-50 rounded-xl border border-blue-100">
          ...
          <div className="flex-1">
            <span className="text-sm font-semibold text-blue-900">Step {currentStep}: {translateUI(steps[currentStep - 1].label)}</span>
            <p className="text-xs text-blue-600">{translateUI(steps[currentStep - 1].desc)}</p>
          </div>
      ...
      {saving && <p className="text-xs text-blue-600" role="status">{translateUI("Menyimpan workflow ke server…")}</p>}
