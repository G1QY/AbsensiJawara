...
export default function CrewEventDashboard({ onStartWorkflow }: { onStartWorkflow: () => void }) {
  ...
  return <div className="p-4 sm:p-6 space-y-4">
    ...
    <div className="grid md:grid-cols-2 gap-3"><Card label="Status Clock In" value={idClock(attendance?.check_in || null)} detail={attendance?.status || 'Belum Clock In'} tone="green"/><Card label="Clock Out" value={idClock(attendance?.check_out || null)} detail={attendance?.check_out ? 'Sudah tercatat' : 'Belum Clock Out'} tone="blue"/></div>
