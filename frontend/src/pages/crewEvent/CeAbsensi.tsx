...
export default function CrewAttendanceHistory({ showAttendance = true }: { showAttendance?: boolean }) {
  ...
  return <div className="p-4 sm:p-6 space-y-5">
    ...
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
      <Summary label={translateUI("Jam telat")} value={`${totalLate} jam`} detail={`Potongan ${money(deduction)}`} tone="red" />
      <Summary label={translateUI("Lembur disetujui")} value={`${totalOvertime} jam`} detail={`Bonus ${money(bonus)}`} tone="green" />
      <Summary label={translateUI("Menunggu persetujuan")} value={pending} detail="Cek notifikasi untuk keputusan admin" tone="amber" />
      <Summary label={translateUI("Bonus − potongan")} value={money(bonus - deduction)} detail="Estimasi dari riwayat absensi" tone="blue" />
    </div>
