...
const money = (n: number) => `Rp${n.toLocaleString(getLocale())}`;
const clock = (iso: string | null) => iso ? new Date(iso).toLocaleTimeString(getLocale(), { timeZone: 'Asia/Jakarta', hour: '2-digit', minute: '2-digit', hourCycle: 'h23' }) : '—';
...
export default function CrewAttendanceHistory({ showAttendance = true }: { showAttendance?: boolean }) {
  ...
  return <div className="p-4 sm:p-6 space-y-5">
    ...
    {loading ? <p className="rounded-xl border border-slate-200 bg-white p-4 text-sm text-slate-500" role="status">{translateUI("Memuat riwayat absensi...")}</p> : null}
    {error ? <p className="ui-error rounded-xl p-4" role="alert">{translateUI(error)}</p> : null}
    <section className="rounded-2xl border border-blue-100 bg-blue-50 p-4 text-sm text-blue-900">
      <strong>{translateUI("Potongan dan bonus milik Anda")}</strong>
      <p className="mt-1 text-xs leading-5">{translateUI("Telat dibulatkan ke atas per jam. Lembur memakai jam penuh dan bonus hanya masuk setelah disetujui admin atau sudah disetujui di jadwal. Kelebihan waktu clock-out tidak otomatis menjadi bonus.")}</p>
    </section>
    <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
      <Summary label={translateUI("Jam telat")} value={`${totalLate} ${translateUI("jam")}`} detail={`${translateUI("Potongan")} ${money(deduction)}`} tone="red" />
      <Summary label={translateUI("Lembur disetujui")} value={`${totalOvertime} ${translateUI("jam")}`} detail={`${translateUI("Bonus")} ${money(bonus)}`} tone="green" />
      <Summary label={translateUI("Menunggu persetujuan")} value={pending} detail={translateUI("Cek notifikasi untuk keputusan admin")} tone="amber" />
      <Summary label={translateUI("Bonus − potongan")} value={money(bonus - deduction)} detail={translateUI("Estimasi dari riwayat absensi")} tone="blue" />
    </div>
    <section className="bg-white border border-slate-200 rounded-2xl overflow-hidden">
      <div className="p-4 border-b border-slate-100 flex flex-wrap items-center justify-between gap-3"><div><h2 className="font-semibold text-slate-900">{translateUI("Riwayat Absensi —") + " "}{auth?.user.full_name}</h2><p className="text-xs text-slate-500 mt-1">{translateUI("Tarif saat ini: potongan") + " "}{money(LATE_RATE)}{translateUI("/jam, bonus") + " "}{money(OVERTIME_RATE)}{translateUI("/jam.")}</p></div><ExportButtons context={{"Nama Crew":auth?.user.full_name||'',"Perusahaan":auth?.user.company_name||'',"Jabatan":auth?.user.job_title||''}} filename="Riwayat-Potongan-Lembur" title={translateUI("Riwayat Potongan dan Lembur")} subtitle={auth?.user.full_name || 'Crew'} headers={['Lokasi/Event','Tanggal','Jadwal','Clock In','Status','Persetujuan Absensi','Clock Out','Jam Telat','Potongan','Jam Lembur','Persetujuan Lembur','Bonus']} rows={rows.map(r=>[r.source,r.date,r.schedule,r.checkIn,r.attendanceStatus,r.attendanceApproval,r.checkOut,r.lateHours,r.deduction,r.overtimeHours,r.overtimeStatus,r.bonus])}/></div>
      <div className="overflow-x-auto"><table className="w-full text-sm"><thead className="bg-slate-50 text-xs text-slate-500"><tr>{['Lokasi / Event','Tanggal & Jadwal','Clock In','Status','Persetujuan Absensi','Clock Out','Potongan Telat','Lembur','Persetujuan Lembur','Bonus'].map(h=><th key={h} className="p-3 text-left whitespace-nowrap">{translateUI(h)}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{rows.map(r=><tr key={r.id}><td className="p-3 font-medium text-slate-900">{r.source}</td><td className="p-3 whitespace-nowrap">{r.date}<small className="block text-slate-500">{r.schedule === 'Belum ada jadwal' ? translateUI(r.schedule) : r.schedule} WIB</small></td><td className="p-3 font-mono">{r.checkIn}</td><td className="p-3"><Badge value={r.attendanceStatus}/></td><td className="p-3"><Badge value={r.attendanceApproval}/></td><td className="p-3 font-mono">{r.checkOut}</td><td className="p-3 text-red-700"><strong>{r.deduction?`-${money(r.deduction)}`:'—'}</strong>{r.lateHours>0&&<small className="block">{r.lateHours}{" " + translateUI("jam")}</small>}</td><td className="p-3">{r.overtimeHours ? `${r.overtimeHours} ${translateUI("jam")}` : '—'}{r.preapproved&&<small className="block text-emerald-700">{translateUI("Dari jadwal")}</small>}</td><td className="p-3"><Badge value={r.overtimeStatus}/></td><td className="p-3 text-emerald-700 font-semibold">{r.bonus?`+${money(r.bonus)}`:'—'}</td></tr>)}</tbody></table></div>
      {!rows.length && <p className="p-8 text-center text-sm text-slate-500">{translateUI("Belum ada riwayat absensi.")}</p>}
    </section>
    <AttendanceCalendar />
  </div>;
}
...
function Summary({label,value,detail,tone}:{label:string;value:string|number;detail:string;tone:'red'|'green'|'amber'|'blue'}) {
  ...
  return <div className={`rounded-2xl p-4 ${color}`}><p className="text-xs opacity-75">{translateUI(label)}</p><strong className="block text-xl mt-1">{value}</strong><small>{detail}</small></div>;
}
function Badge({value}:{value:string}) {
  ...
  return <span className={`inline-flex px-2 py-1 rounded-full text-xs font-semibold whitespace-nowrap ${color}`}>{translateUI(value)}</span>;
}
