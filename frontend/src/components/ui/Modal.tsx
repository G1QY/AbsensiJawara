...
export default function Modal({ open, onClose, title, children, size = 'md' }: ModalProps) {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      ...
      <div role="dialog" aria-modal="true" aria-label={translateUI(title)} className={`relative bg-white rounded-2xl shadow-2xl w-full max-h-[90dvh] flex flex-col ${sizeMap[size]} animate-fade-in`}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-900">{translateUI(title)}</h2>
          <button
            aria-label={translateUI("Tutup dialog")}
            onClick={onClose}
...
export function ConfirmDialog({ open, onClose, onConfirm, title, message, confirmLabel = 'Konfirmasi', danger = false }: {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" onClick={onClose} />
      <div role="dialog" aria-modal="true" aria-label={translateUI(title)} className="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm animate-fade-in p-6">
        ...
        <h3 className="text-base font-semibold text-slate-900 mb-1">{translateUI(title)}</h3>
        <p className="text-sm text-slate-500 mb-6">{translateUI(message)}</p>
        <div className="flex gap-3">
          <button onClick={onClose} className="flex-1 px-4 py-2 rounded-xl text-sm font-medium border border-slate-200 text-slate-600 hover:bg-slate-50">{" " + translateUI("Batal") + " "}</button>
