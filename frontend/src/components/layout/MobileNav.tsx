...
export default function MobileNav({role,currentPage,onNavigate,onMenu,menuOpen}:Props){
 ...
 return <nav className="mobile-employee-nav md:hidden" aria-label={translateUI("Navigasi karyawan")}>{[...items,['menu','Menu','menu']].map(([id,label,icon])=>{const active=id===currentPage||(id==='ce-events'&&currentPage==='ce-workflow');return <button key={id} type="button" aria-current={active?'page':undefined} aria-expanded={id==='menu'?menuOpen:undefined} onClick={()=>id==='menu'?onMenu():onNavigate(id)}><svg aria-hidden="true" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d={paths[icon as keyof typeof paths]}/></svg><span>{translateUI(label)}</span></button>})}</nav>;
}
