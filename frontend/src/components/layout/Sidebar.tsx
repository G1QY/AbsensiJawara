    ...
    return (
      <>
        ...
        <div className={`flex items-center border-b border-white/5 flex-shrink-0 ${role === 'guest_crew' ? 'h-11 px-3' : 'h-16 px-4'}`}>
          ...
          >
            <span className="font-bold text-sm" aria-hidden="true">J</span>
          </button>
          {!isCollapsed && (
            <span className={`sidebar-brand font-bold tracking-tight whitespace-nowrap overflow-hidden ${role === 'guest_crew' ? 'ml-2 text-sm' : 'ml-3 text-lg'}`}>JAWARA</span>
          )}
          {forceExpanded && (
