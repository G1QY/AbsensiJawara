...
router.get('/',async(req,res,next)=>{try{
  ...
  const {data:assignment,error:assignmentError}=await db.from('store_assignments').select('id,status,start_date,end_date,store:stores(id,name,status,branch:branches(name,city_name))').eq('crew_id',crew.id).eq('status','ACTIVE').lte('start_date',workingDate).or(`end_date.is.null,end_date.gte.${workingDate}`).maybeSingle();
