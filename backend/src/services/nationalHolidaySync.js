...
async function fetchGoogleHolidayRows(year, signal) {
  ...
  return { rows: normalizeRows(items, year), provider: `google-calendar:${calendarId}` };
}
...
async function syncHolidayYear(value, { force = false } = {}) {
  ...
  if (!force && cached && new Date(cached.synced_at).getTime() >= freshAfter) {
    return { ...cached, cached: true, fallback: false };
  }

  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 10000);
  let provider = `google-calendar:${process.env.GOOGLE_HOLIDAY_CALENDAR_ID || DEFAULT_CALENDAR_ID}`;
  try {
    ...
    if (await hasCachedRows(year)) {
      return { ...(cached || { year, provider_url: provider, synced_at: null, row_count: null }), cached: true, fallback: true, warning: error.message };
    }
