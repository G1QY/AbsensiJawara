...
const db={rpc:async(name,args)=>{decisions.push({name,args});return {error:null};},from(table){let inserted=null;let filter=null;
  const q={select(){return q;},eq(k,v){filter={k,v};return q;},in(){return q;},order(){return q;},range(){return q;},insert(row){inserted=row;return q;},
    ...
    is(){return q;}, then(resolve,reject){return Promise.resolve({data:table==='guest_attendances'?saved:[],error:null}).then(resolve,reject);}};return q;}};
...
test('guest submission and private admin review routes',async()=>{
  ...
  try{
    ...
    const deleted=await fetch(url+'/admin-attendance/guest/'+g.id,{method:'DELETE',headers:{...admin,'Content-Type':'application/json'},body:JSON.stringify({reason:'Data uji'})});
    assert.equal(deleted.status,200);assert.equal(decisions.at(-1).name,'delete_admin_attendance');assert.equal(decisions.at(-1).args.p_actor,id);assert.equal(decisions.at(-1).args.p_id,g.id);
    assert.equal((await fetch(url+'/admin-attendance/guest/'+g.id,{method:'DELETE',headers:{...admin,'Content-Type':'application/json'},body:JSON.stringify({reason:''})})).status,422);
    assert.equal((await fetch(url+'/admin-attendance/guest/'+g.id,{method:'DELETE'})).status,403);
